...
* To add a variable that changes over time, use `pick`. As an example, degree class is `DEGCLS7` from 2007 onwards, and `DEGCLS` before then. This code imports those as a factor (retaining the SPSS labels), and calls the new column `DEGREE_CLASS`.
...
user_extra_mappings <- function(lfs_file_column_names) {
  ...
  degree_class <- pick(c("DEGCLS7", "DEGCLS"), lfs_file_column_names)
